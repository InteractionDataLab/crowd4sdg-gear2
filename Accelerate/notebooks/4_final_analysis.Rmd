...
final = read.csv("../data/final_form_accelerate.csv")
...
#temp_compr$variable = gsub("\\.", " ", temp_compr$variable)

plt = ggplot(temp_compr[!grepl("since I participated", temp_compr$variable),], aes(x = mean, y = reorder(variable, mean))) + geom_point() + theme_bw(base_size = 15) + geom_errorbar(aes(xmin = mean-se, xmax = mean+se), width = 0) + xlab("Response Score") + ylab("Questions") + ggtitle("To which extend do you agree to the following statements") + scale_x_continuous(labels=names(list), breaks=-2:2, limits=c(-2,2))+ xlab("Scores")

ggsave(plt, filename = "../figures/which_extent_1.png", height = 7, width = 20)


#plt = ggplot(temp[!grepl("since.I.participated", temp$variable),], aes(x = score, y = reorder(variable, score))) + geom_boxplot() + geom_point(aes(x = score, y = reorder(variable, score))) + theme_bw(base_size = 15) + xlab("Response Score") + ylab("Questions") + ggtitle("To which extend do you agree to the following statements") + scale_x_continuous(labels=names(list), breaks=-2:2, limits=c(-2,2))+ xlab("Scores")

###############################

temp_compr$y_new = str_replace(temp_compr$variable, "since I participated in the CBIW  ", "")

plt = ggplot(temp_compr[grepl("since I participated", temp_compr$variable),], aes(x = mean, y = reorder(y_new, mean))) + geom_point() + theme_bw(base_size = 15) + geom_errorbar(aes(xmin = mean-se, xmax = mean+se), width = 0) + xlab("Response Score") + ylab("Questions") + ggtitle("To which extend do you agree to the following statements") + scale_x_continuous(labels=names(list), breaks=-2:2, limits=c(-2,2))+ xlab("Scores")

ggsave(plt, filename = "../figures/which_extent_2.png", height = 7, width = 20)
...
#temp_agg$variable = gsub("\\.", " ", temp_agg$variable)
...
temp_agg$y_new = str_replace(temp_agg$variable, "since I participated in the CBIW  ", "")
...
temp$variable = str_replace(temp$variable, "What.communication.tool.s..did.you.use.during.CBIW...", "")
temp$variable = gsub("\\.", " ", temp$variable)

temp_compr = temp %>% group_by(variable) %>% summarise(mean = mean(score, na.rm = TRUE), se = se(score))

plt = ggplot(temp_compr, aes(x = mean, y = reorder(variable, mean))) + geom_point() + theme_bw(base_size = 15) + geom_errorbar(aes(xmin = mean-se, xmax = mean+se), width = 0)  + ylab("Questions") + ggtitle("What communication tools did you use during CBIW") + scale_x_continuous(labels=names(list), breaks=0:4, limits=c(0,4))+ xlab("Scores")
...
temp$variable = str_replace(temp$variable, "Did.you.have.issues.connecting.with.any.of.the.platforms.used.during.CBIW...", "")
temp$variable = gsub("\\.", " ", temp$variable)

temp_compr = temp %>% group_by(variable) %>% summarise(mean = mean(score, na.rm = TRUE), se = se(score))

plt = ggplot(temp_compr, aes(x = mean, y = reorder(variable, mean))) + geom_point() + theme_bw(base_size = 15) + geom_errorbar(aes(xmin = mean-se, xmax = mean+se), width = 0) + ylab("Questions") + ggtitle("Did you have issues connecting with any of the platforms used during CBIW") + scale_x_continuous(labels=names(list), breaks=0:4, limits=c(0,4))+ xlab("Scores")

ggsave(plt, filename = "../figures/did_issues.png", height = 7, width = 10)
...
plt = ggplot(temp_compr, aes(x = mean, y = reorder(variable, mean))) + geom_point() + theme_bw(base_size = 15) + geom_errorbar(aes(xmin = mean-se, xmax = mean+se), width = 0) + ylab("Questions") + ggtitle("How often did you connect with these people for feedback and support for your team project") + scale_x_continuous(labels=names(list), breaks=0:4, limits=c(0,4)) + xlab("Scores")

ggsave(plt, filename = "../figures/how_often.png", height = 7, width = 10)
...
list = c("Very dissatisfied" = 0, "Dissatisfied" = 1, "Neither dissatisfied or satisfied" = 2, "Satisfied" = 1, "Very satisfied" = 2)

feedback = "Please.rate.your.overall.satisfaction.with.the.quality.of.the.CBIW"
...
plt = ggplot(temp, aes(y = value)) + geom_bar(stat = "count") + theme_bw(base_size = 15) + xlab("Response Score") + ylab("Questions")  + ggtitle("Satisfaction with the quality of the CBIW") + scale_y_discrete(drop = FALSE)
...
vertices = data.frame(node = unique(temp$What.is.your.team.), type = TRUE)
vertices = rbind(vertices, data.frame(node = unique(df_tools$tools), type = FALSE))
...
visPlot(g_tools_simp, textsize = 2*strength(g_tools_simp), directed = TRUE, nodesize = strength(g_tools_simp), edgewidth = E(g_tools_simp)$weight, communities = V(g_tools_simp)$type+0)
...
save(cs_tools, sat, d_i, w_e, w_c, h_o, ext, file = "../processed data/comparison_accelerate.RData")
